"""SistemaHidrico query functions - core API interaction for OpenClaw.

This module provides all the functions OpenClaw needs to interact with
the SistemaHidrico OSINT platform.

For local access (same machine), no authentication is needed.
For remote access, set SISTEMAHIDRICO_HMAC_SECRET to enable HMAC-signed requests.

Usage (inside an OpenClaw skill):
    from sb_query import SistemaHidricoClient
    sb = SistemaHidricoClient()
    data = await sb.get_telemetry()
    await sb.place_pin(34.05, -118.24, "UAP Sighting", category="anomaly")

Remote usage:
    import os
    os.environ["SISTEMAHIDRICO_URL"] = "https://your-server.com:8000"
    os.environ["SISTEMAHIDRICO_HMAC_SECRET"] = "your-hmac-secret-here"
    sb = SistemaHidricoClient()
"""
...
SB_BASE = os.environ.get("SISTEMAHIDRICO_URL", "http://127.0.0.1:8000")


class SistemaHidricoClient:
    """Client for the SistemaHidrico REST API.

    Supports both local (no auth) and remote (HMAC-signed) connections.
    Set SISTEMAHIDRICO_HMAC_SECRET env var to enable remote authentication.
    """

    def __init__(self, base_url: str = SB_BASE, hmac_secret: str = ""):
        self.base = base_url.rstrip("/")
        self._hmac_secret = hmac_secret or os.environ.get("SISTEMAHIDRICO_HMAC_SECRET", "")
        self._client = None
    ...
    # Patterns that look like LLM API keys — never send these to SistemaHidrico.
    ...
    def _sanitize_payload(cls, data: dict) -> dict:
        """Scrub LLM API keys from payloads before sending to SistemaHidrico.
    ...
    async def poll_channel(self) -> dict:
        """Poll for command responses and tasks from SistemaHidrico.
    ...
    async def inject_data(
        ...
        """Inject custom data into a native SistemaHidrico layer."""
    ...
    async def get_full_telemetry(self) -> dict:
        ...
        This gives the agent access to every layer SistemaHidrico tracks:
    ...
    async def stream_updates(
        ...
        *,
        ...
        SistemaHidrico.  The server pushes:
