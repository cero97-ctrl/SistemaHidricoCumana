...
name: sistemahidrico
description: >
  Query the SistemaHidrico OSINT intelligence platform for real-time geospatial
...
# SistemaHidrico Intelligence Skill

You have access to **SistemaHidrico**, a real-time global OSINT intelligence platform
...
sb = SistemaHidricoClient()  # auto-detects local or remote mode
```
...
```bash
SISTEMAHIDRICO_URL=https://your-sistemahidrico-host:8000
SISTEMAHIDRICO_HMAC_SECRET=your-hmac-secret-here
```

The HMAC secret is found in SistemaHidrico's **Connect OpenClaw** modal (AI Intel panel).
...
Inject custom data directly into SistemaHidrico's native layers (CCTV, ships, etc.):
...
await sb.send_mesh("US", "SistemaHidrico AI: SIGINT anomaly detected in sector 7")
...
SistemaHidrico can ingest free SAR data in two modes:
...
|--------------|--------|-------------|
| `brief` | 🌍📡 SISTEMAHIDRICO BRIEF: | Morning/evening intelligence digest |
| `warning` | 🌍⚠️ SISTEMAHIDRICO WARNING: | Life-safety alert (earthquake, weather emergency) |
| `news` | 🌍📰 SISTEMAHIDRICO NEWS: | Breaking news alert |
| `intel` | 🌍🛰️ SISTEMAHIDRICO INTEL: | Intelligence update (carrier movement, military buildup) |
| `searching` | 🌍🔍 SISTEMAHIDRICO SEARCHING: | Search/query in progress |
| `pinning` | 🌍📌 SISTEMAHIDRICO PINNING: | Placing pins on the map |
| `markets` | 🌍📊 SISTEMAHIDRICO MARKETS: | Prediction market or financial alert |
| `sigint` | 🌍📻 SISTEMAHIDRICO SIGINT: | SIGINT/RF anomaly |
| `threat` | 🌍🔴 SISTEMAHIDRICO THREAT: | Threat level change |
| `near_you` | 🌍📍 SISTEMAHIDRICO NEAR YOU: | Proximity-based event |
| `tracking` | 🌍🎯 SISTEMAHIDRICO TRACKING: | Tracking a specific entity |
| `correlation` | 🌍⚡ SISTEMAHIDRICO CORRELATION: | Cross-layer correlation |
| `seismic` | 🌍🌋 SISTEMAHIDRICO SEISMIC: | Earthquake/volcanic activity |
| `fire` | 🌍🔥 SISTEMAHIDRICO FIRE: | FIRMS fire hotspot |
| `flight` | 🌍🛫 SISTEMAHIDRICO FLIGHT: | Military/tracked flight alert |
| `maritime` | 🌍🚢 SISTEMAHIDRICO MARITIME: | Ship/carrier event |
| `weather` | 🌍🌤️ SISTEMAHIDRICO WEATHER: | Weather alert |
| `sar` | 🌍📡 SISTEMAHIDRICO SAR: | Synthetic aperture radar anomaly (deformation, flood, damage) |
| `online` | 🌍✅ SISTEMAHIDRICO ONLINE: | System connected |
| `clearing` | 🌍❌ SISTEMAHIDRICO CLEARING: | Pins/data cleared |
...
2. **Does SistemaHidrico have this data already?**
