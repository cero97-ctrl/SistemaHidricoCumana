...
// "native window only" trust model for `__SISTEMAHIDRICO_DESKTOP__`.
const DESKTOP_INIT_SCRIPT: &str = r#"
(function() {
  if (typeof window === 'undefined') return;
  if (window.__SISTEMAHIDRICO_DESKTOP__) return; // idempotent on navigation
  ...
  window.__SISTEMAHIDRICO_DESKTOP__ = {
...
fn main() {
    let explicit_backend_url = std::env::var("SISTEMAHIDRICO_BACKEND_URL").ok();
    let admin_key = std::env::var("SISTEMAHIDRICO_ADMIN_KEY").ok();

    // Frontend URL detection:
    // - If SISTEMAHIDRICO_FRONTEND_URL is explicitly set → honor it (dev mode
    ...
    let frontend_url_explicit = std::env::var("SISTEMAHIDRICO_FRONTEND_URL").ok();
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            // user has NOT explicitly set SISTEMAHIDRICO_BACKEND_URL, the app:
            ...
            let (resolved_backend_base_url, owns_managed_backend, resolved_admin_key) =
                if let Some(url) = explicit_backend_url.as_ref() {
                    ...
                    match tauri::async_runtime::block_on(
                        ...
                        Err(e) => {
                            return Err(format!(
                                "SistemaHidrico cannot start: the bundled local backend failed to launch.\n\n\
                    ...
                    return Err(
                        "SistemaHidrico cannot start: this packaged build is missing the bundled backend runtime."
                            .into(),
                    );
                } else {
                ...
                && frontend_url_explicit.is_none()
            {
                ...
                match tauri::async_runtime::block_on(async move {
                    ...
                    Err(e) => {
                        ...
                        return Err(format!(
                            "SistemaHidrico cannot start: the packaged loopback server failed to bind.\n\n\
            ...
            //       so the __SISTEMAHIDRICO_DESKTOP__ native control bridge is
            ...
            //   2. Explicit SISTEMAHIDRICO_FRONTEND_URL → that URL
        ...
        .expect("failed to build sistemahidrico tauri shell")
        .run(|app, event| {
