...
//! receive the native desktop control boundary (`window.__SISTEMAHIDRICO_DESKTOP__`)
...
pub struct CompanionState {
    enabled: bool,
    /// Default frontend URL (from `SISTEMAHIDRICO_FRONTEND_URL` or the
    /// `http://127.0.0.1:3000` fallback used in dev mode).
    default_frontend_url: String,
    /// Whether `SISTEMAHIDRICO_FRONTEND_URL` was explicitly set by the user.
...
mod tests {
    ...
    fn explicit_override_beats_app_server() {
        ...
        assert_eq!(
            cs.effective_url(),
            "http://127.0.0.1:4000",
            "explicit SISTEMAHIDRICO_FRONTEND_URL must beat the built-in server URL"
        );
        assert!(!cs.uses_builtin_server());
    }
