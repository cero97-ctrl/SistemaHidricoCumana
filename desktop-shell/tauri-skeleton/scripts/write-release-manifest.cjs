...
const updateBaseUrl = (
  process.env.SISTEMAHIDRICO_UPDATE_BASE_URL ||
  'https://github.com/cero97-ctrl/SistemaHidricoCumana/releases/latest/download'
).replace(/\/+$/, '');
...
function writeUpdaterManifest(files) {
  ...
  const latest = {
    ...
    notes: `SistemaHidrico ${readReleaseVersion()}`,
