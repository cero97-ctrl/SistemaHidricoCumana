...
#   - Backend running on :8000 (or set SISTEMAHIDRICO_BACKEND_URL)
#
# Usage:
#   ./dev.sh                                # default backend at http://127.0.0.1:8000
#   SISTEMAHIDRICO_ADMIN_KEY=secret ./dev.sh  # with admin key for privileged commands
#
# This script starts Tauri in dev mode, which:
#   1. Opens a native window pointed at the frontend dev server (http://127.0.0.1:3000)
#   2. Injects window.__SISTEMAHIDRICO_DESKTOP__ for native command routing
...
export SISTEMAHIDRICO_BACKEND_URL="${SISTEMAHIDRICO_BACKEND_URL:-http://127.0.0.1:8000}"

echo "=== SistemaHidrico Tauri Dev Shell ==="
echo "Backend URL: $SISTEMAHIDRICO_BACKEND_URL"
echo "Admin key:   ${SISTEMAHIDRICO_ADMIN_KEY:+(set)}"
echo ""
echo "Make sure the frontend dev server is running on http://127.0.0.1:3000"
echo "Make sure the backend is running on $SISTEMAHIDRICO_BACKEND_URL"
echo ""
