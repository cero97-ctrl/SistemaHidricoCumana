...
Cross-platform Tauri integration for the SistemaHidrico desktop boundary.
...
1. `main.rs` creates the main window programmatically and attaches an
   `initialization_script` so `window.__SISTEMAHIDRICO_DESKTOP__` exists before
...
- `SISTEMAHIDRICO_BACKEND_URL` - Optional backend override. In packaged mode, if unset, the app launches its bundled local backend automatically.
- `SISTEMAHIDRICO_ADMIN_KEY` - Optional admin key for privileged backend access
- `SISTEMAHIDRICO_FRONTEND_URL` - Explicit frontend origin override for dev/custom setups
