...
//!   the menu; "Show SistemaHidrico" restores the window. "Quit" exits fully.
...
pub fn setup_tray(app: &AppHandle) -> Result<TrayIcon, String> {
    let show_item = MenuItem::with_id(app, MENU_ID_SHOW, "Show SistemaHidrico", true, None::<&str>)
    ...
    let quit_item = MenuItem::with_id(app, MENU_ID_QUIT, "Quit SistemaHidrico", true, None::<&str>)
...
///   background. The user can restore via tray menu or quit via "Quit SistemaHidrico".
