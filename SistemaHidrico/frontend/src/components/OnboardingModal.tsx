...
const STORAGE_KEY = `sistemahidrico_onboarding_complete_v${CURRENT_ONBOARDING_VERSION}`;
const LEGACY_STORAGE_KEY = 'sistemahidrico_onboarding_complete';
...
const OnboardingModal = React.memo(function OnboardingModal({
  ...
  const agentSnippet = [
    `SISTEMAHIDRICO_URL=${agentEndpoint}`,
    agentSecret ? `SISTEMAHIDRICO_KEY=${agentSecret}` : 'SISTEMAHIDRICO_KEY=<generate in SistemaHidrico>',
    `SISTEMAHIDRICO_ACCESS=${agentTier}`,
    '',
  ...
  const prepareTorAgentAddress = async () => {
    ...
    try {
      ...
      setAgentMsg({
        type: 'ok',
        text: 'Tor is ready. The remote agent link is private to your local SistemaHidrico node.',
      });
    } catch (error) {
      setAgentMsg({
        type: 'err',
        text:
          error instanceof Error
            ? error.message
            : 'SistemaHidrico could not install or start Tor automatically. Check network access and try again.',
      });
    } finally {
                      ...
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          ...
                          <p className="text-[10px] text-[var(--text-muted)] font-mono mt-1 leading-relaxed">
                            SistemaHidrico will install or use Tor locally, then create a private .onion link for this backend.
                          </p>
                        </div>
                        <button
                ...
                <div className="bg-yellow-950/20 border border-yellow-500/20 p-4">
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-[var(--text-secondary)] font-mono leading-relaxed">
                        OpenSky Network and AIS Stream are the free keys that make SistemaHidrico
                ...
                <div className="border border-cyan-900/40 bg-cyan-950/10 p-4 space-y-3">
                  <div>
                    ...
                    <p className="text-sm text-[var(--text-secondary)] font-mono leading-relaxed mt-1">
                      Paste keys here once. SistemaHidrico stores them server-side only and never
