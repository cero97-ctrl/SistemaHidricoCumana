...
export default function ShodanPanel({
  ...
  const exportPresets = useCallback(() => {
    downloadText(
      `sistemahidrico-shodan-presets-${new Date().toISOString().slice(0, 10)}.json`,
      JSON.stringify({ source: 'SistemaHidrico', type: 'shodan-presets', presets }, null, 2),
    );
  }, [presets]);
  ...
  const exportResultsJson = useCallback(() => {
    ...
    downloadText(
      `sistemahidrico-shodan-results-${new Date().toISOString().replace(/[:.]/g, '-')}.json`,
      JSON.stringify(
        {
          source: 'Shodan',
  ...
  const exportResultsCsv = useCallback(() => {
    if (!currentResults.length) return;
    downloadText(
      `sistemahidrico-shodan-results-${new Date().toISOString().replace(/[:.]/g, '-')}.csv`,
