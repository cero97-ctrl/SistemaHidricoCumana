...
export default function AIQueryView({ onBack }: AIQueryViewProps) {
  const [messages, setMessages] = useState<AIMessage[]>([
    {
      role: 'system',
      content: `🌍📡 SISTEMAHIDRICO AI CO-PILOT ONLINE
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Connected to SistemaHidrico OSINT platform.
  ...
  • "help"    — This message`,
    ...
    try {
      ...
      if (lowerQuery === 'status') {
        ...
        return {
          content: `🌍✅ SISTEMAHIDRICO AI STATUS:
━━━━━━━━━━━━━━━━━━━━━━━━━━━
      ...
      if (lowerQuery.startsWith('pin ')) {
        ...
        if (parts.length >= 3) {
          ...
          if (!isNaN(lat) && !isNaN(lng)) {
            ...
            const data = await resp.json();
            return {
              content: `🌍📌 SISTEMAHIDRICO PINNING:
...
${pinList || '  No pins placed yet.'}`,
      ...
      if (lowerQuery === 'clear pins') {
        ...
        return { content: '🌍❌ SISTEMAHIDRICO CLEARING:\nAll AI intel pins cleared.' };
      }
      ...
      if (lowerQuery === 'snapshot' || lowerQuery === 'take snapshot') {
        ...
        const data = await resp.json();
        return {
          content: `🌍🕰️ SISTEMAHIDRICO TIMEMACHINE:
Snapshot taken!
  🆔 ${data.snapshot_id}
  🕐 ${data.timestamp}
...
${alerts}`,
      ...
      return {
        content: `🌍🔍 SISTEMAHIDRICO SEARCHING:
...
Type "help" for the full command list.`,
      ...
      return {
        content: `🌍⚠️ SISTEMAHIDRICO WARNING:
Query failed: ${error instanceof Error ? error.message : 'Unknown error'}
Make sure the SistemaHidrico backend is running on localhost:8000.`,
      };
    }
  }, []);
      ...
      {showConnect && (
        <div className="border-b border-purple-900/40 bg-purple-950/15 px-4 py-4 shrink-0 overflow-y-auto max-h-[60vh]">
          ...
          <div className="space-y-3 text-sm font-mono">
            {/* API Endpoint */}
            <div>
              <div className="text-[11px] text-gray-500 uppercase tracking-widest mb-1">Your SistemaHidrico API Endpoint</div>
              <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <div className="bg-black/60 border border-gray-800/40 rounded-sm p-3 space-y-2 text-[13px] leading-relaxed">
                <p className="text-cyan-400 font-bold">Step 1: Install the SistemaHidrico Skill</p>
                <p className="text-gray-400">Copy the <code className="text-purple-300 bg-purple-900/30 px-1">openclaw-skills/sistemahidrico/</code> folder into your OpenClaw&apos;s skills directory.</p>
                ...
                <code className="block bg-purple-950/40 border border-purple-800/30 px-2 py-1 text-purple-300 text-[13px] rounded-sm">
                  SISTEMAHIDRICO_URL={apiEndpoint}
                </code>
                ...
                <div className="relative">
                  <pre className="bg-purple-950/40 border border-purple-800/30 px-2 py-2 text-[12px] text-purple-200 rounded-sm overflow-x-auto whitespace-pre-wrap">{`You have a skill called "sistemahidrico" that connects you to a real-time global OSINT intelligence platform. Use it to:
...
API: ${apiEndpoint}
Skill docs: openclaw-skills/sistemahidrico/SKILL.md`}</pre>
                  <button
                    onClick={() => handleCopy(`You have a skill called "sistemahidrico" that connects you to a real-time global OSINT intelligence platform. Use it to:\n- Query military flights, ships, satellites, SIGINT, earthquakes, and 30+ data layers\n- Place intelligence pins on a live map\n- Fetch satellite imagery from Sentinel-2\n- Aggregate news by region via GDELT\n- Take telemetry snapshots (Time Machine)\n- Participate in the Wormhole encrypted mesh network\n- Send/receive InfoNet messages via decentralized feed\n\nAPI: ${apiEndpoint}\nSkill docs: openclaw-skills/sistemahidrico/SKILL.md`)}
