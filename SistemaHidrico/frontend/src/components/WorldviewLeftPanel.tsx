...
const WorldviewLeftPanel = React.memo(function WorldviewLeftPanel({
  ...
  const [sarChoice, setSarChoice] = useState<import('./SarModeChooserModal').SarChoice>(() => {
    try {
      const stored = localStorage.getItem('sistemahidrico_sar_mode_choice');
    ...
    const check = async () => {
      try {
        ...
        if (modeBOn && sarChoice !== 'b_active') {
          try {
            localStorage.setItem('sistemahidrico_sar_mode_choice', 'b_active');
          } catch {
            // ignore
          }
          ...
          try {
            localStorage.removeItem('sistemahidrico_sar_mode_choice');
          } catch {
            // ignore
          }
          setSarChoice(null);
