...
import type { SistemaHidricoDesktopRuntime } from '@/lib/desktopBridge';

describe('desktopBridgeBootstrapPreference', () => {
  beforeEach(() => {
    vi.resetModules();
    // Clean window globals before each test
    delete (window as Record<string, unknown>).__SISTEMAHIDRICO_DESKTOP__;
    delete (window as Record<string, unknown>).__SISTEMAHIDRICO_LOCAL_CONTROL__;
  });

  it('prefers a pre-installed native runtime over the HTTP shim', async () => {
    const nativeInvoke = vi.fn().mockResolvedValue({ ok: true });
    const nativeRuntime: SistemaHidricoDesktopRuntime = {
    ...
    // Simulate Tauri injection: set __SISTEMAHIDRICO_DESKTOP__ before bootstrap
    window.__SISTEMAHIDRICO_DESKTOP__ = nativeRuntime;
    ...
    expect(window.__SISTEMAHIDRICO_LOCAL_CONTROL__).toBeDefined();
    expect(window.__SISTEMAHIDRICO_LOCAL_CONTROL__!.invoke).toBeDefined();

    // Invoke through the bridge — should delegate to the native runtime
    await window.__SISTEMAHIDRICO_LOCAL_CONTROL__!.invoke!({
  ...
  it('does not install bridge when no native runtime and shim env is off', async () => {
    // No __SISTEMAHIDRICO_DESKTOP__ and NEXT_PUBLIC_ENABLE_DESKTOP_BRIDGE_SHIM != '1'
    ...
    expect(window.__SISTEMAHIDRICO_LOCAL_CONTROL__).toBeUndefined();
  ...
  it('falls back to HTTP shim when no native runtime and shim env is on', async () => {
    ...
    expect(window.__SISTEMAHIDRICO_LOCAL_CONTROL__).toBeDefined();
    // __SISTEMAHIDRICO_DESKTOP__ is the HTTP-backed shim
    expect(window.__SISTEMAHIDRICO_DESKTOP__).toBeDefined();
    expect(window.__SISTEMAHIDRICO_DESKTOP__!.invokeLocalControl).toBeDefined();
    expect(window.__SISTEMAHIDRICO_DESKTOP__!.getNativeControlAuditReport).toBeDefined();
    expect(window.__SISTEMAHIDRICO_DESKTOP__!.clearNativeControlAuditReport).toBeDefined();
  ...
  it('native runtime rejects on capability mismatch and records audit', async () => {
    ...
    await expect(
      window.__SISTEMAHIDRICO_LOCAL_CONTROL__!.invoke!({
        command: 'wormhole.gate.key.rotate',
