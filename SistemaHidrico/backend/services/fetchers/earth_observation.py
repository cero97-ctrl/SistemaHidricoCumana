...
def fetch_weather_alerts():
    ...
    try:
        ...
        headers = {
            "User-Agent": "(SistemaHidrico OSINT Dashboard, github.com/cero97-ctrl/SistemaHidricoCumana)",
...
_NUFORC_LIVE_USER_AGENT = "Mozilla/5.0 (SistemaHidrico-OSINT NUFORC-fetcher)"
...
def _photon_lookup(query: str) -> list[float] | None:
    ...
    try:
        res = fetch_with_curl(
            url,
            headers={
                "User-Agent": "SistemaHidrico-OSINT/1.0 (NUFORC-UAP-layer)",
                "Accept-Language": "en",
            },
            timeout=10,
        )
