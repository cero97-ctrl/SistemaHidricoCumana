"""SistemaHidrico autonomous monitoring agent — heartbeat & anomaly detection.

Runs on OpenClaw's scheduling system. On each heartbeat:
  1. Pull telemetry from SistemaHidrico
...
def add_custom_feed(
    ...
    Args:
        ...
        target_layer: SistemaHidrico layer to inject into (cctv, ships, news, etc.)
...
async def _fetch_feed(feed: CustomFeed) -> list[dict]:
    ...
    async with httpx.AsyncClient(timeout=15) as client:
        resp = await client.get(feed.url, headers={
            "User-Agent": "SistemaHidrico-OSINT/1.0 (custom-feed)",
        })
