...
[![SistemaHidrico](/uploads/46f99d19fa141a2efba37feee9de8aab/Title.jpg)](https://github.com/user-attachments/assets/248208ec-62f7-49d1-831d-4bd0a1fa6852)





**SistemaHidrico** is a decentralized intelligence platform that aggregates real-time, multi-domain OSINT telemetry from 60+ live intelligence feeds into a single dark-ops map interface. Aircraft, ships, satellites, conflict zones, CCTV networks, GPS jamming, internet-connected devices, police scanners, mesh radio nodes, and breaking geopolitical events — all updating in real time on one screen as well as an obfuscated communications protocol and information exchange infrastructure.
...
A surprising amount of global telemetry is already public — aircraft ADS-B broadcasts, maritime AIS signals, satellite orbital data, earthquake sensors, mesh radio networks, police scanner feeds, environmental monitoring stations, internet infrastructure telemetry, and more. This data is scattered across dozens of tools and APIs. SistemaHidrico combines all of it into a single interface.
...
SistemaHidrico includes an optional Shodan connector for operator-supplied API access. Shodan results are fetched with your own `SHODAN_API_KEY`, rendered as a local investigative overlay (not merged into core feeds), and remain subject to Shodan’s terms of service.
...
> **Blank news/UAP/bases/wastewater after several minutes?** Check for backend OOM restarts with `docker events --since 30m --filter container=sistemahidrico-backend --filter event=oom`. The default compose file gives the backend 4GB; if your host has less memory, reduce enabled feeds or set `BACKEND_MEMORY_LIMIT=3G` and expect slower/heavier layers to warm more gradually.

> **Podman users:** Podman works, but `podman compose` is a wrapper and still needs a Compose provider installed. On Windows/WSL, if you see `looking up compose provider failed`, install `podman-compose` and run `podman-compose pull` followed by `podman-compose up -d` from inside the cloned `SistemaHidrico` folder. On Linux/macOS/WSL shells you can also use `./compose.sh --engine podman pull` and `./compose.sh --engine podman up -d`.

---

##  🔄 **How to Update**

SistemaHidrico uses pre-built Docker images — no local building required. Updating takes seconds:
...
For high-availability deployments or home-lab clusters, SistemaHidrico supports deployment via **Helm**. This chart is based on the `bjw-s-labs` template and provides a robust, modular setup for both the backend and frontend.
...
helm install sistemahidrico ./helm/chart --create-namespace --namespace sistemahidrico
```
...
* **InfoNet Experimental Testnet** — A global, obfuscated message relay. Anyone running SistemaHidrico can transmit and receive on the InfoNet. Messages pass through a Wormhole relay layer with gate personas, Ed25519 canonical payload signing, and transport obfuscation.
...
* **Internet Device Search** — Query Shodan directly from SistemaHidrico. Search by keyword, CVE, port, or service — results plotted as a live overlay on the map
...
![Gaza](https://gitlab.com/cero97-ctrl/SistemaHidrico/uploads/c55a0c8d49e5e05c6cd094279e6e089b/gaza-screenshot.jpg)

### 🤖 Agentic AI Command Channel — OpenClaw + Compatible Agents (expanded in v0.9.7)

SistemaHidrico exposes a **bidirectional agentic AI command channel** — a signed, tier-gated bridge that gives any compatible AI agent full read/write access to the intelligence platform. **OpenClaw is the reference agent**, but the channel is an open protocol: any LLM-driven agent that signs requests with HMAC-SHA256 (Claude Code, GPT, LangChain, custom Python/TypeScript clients, or your own integration) can connect as an analyst that sees the same data as the operator and can take actions on the map. SistemaHidrico does *not* bundle an LLM, an agent runtime, or model weights — it provides the surface; you bring the agent.

v0.9.7 turns SistemaHidrico from a dashboard a human watches into an intelligence surface any agent can act on.
...
* **Native Layer Injection** — Push custom data directly into SistemaHidrico's native layers (CCTV cameras, ships, SIGINT nodes, military bases, etc.) so agent-discovered sources render alongside real feeds.
...
╚═════════════════════════════════════════════════════════════════════════════╝
   ...
   Distribution
   ────────────
     GitHub (primary):  ghcr.io/cero97-ctrl/sistemahidrico-{backend,frontend}
     GitLab (mirror):   ghcr.io/cero97-ctrl/sistemahidrico/{backend,frontend}
...
**Podman users:** Do not pass the GitHub URL to `podman compose pull`; clone the repo first, `cd SistemaHidrico`, then run compose from that folder. `podman compose` also requires a Compose provider. If Podman reports `looking up compose provider failed`, install one:
...
##   GitHub  (primary): ghcr.io/cero97-ctrl/sistemahidrico-backend:latest
##   GitLab  (mirror):  ghcr.io/cero97-ctrl/sistemahidrico/backend:latest


services:
  backend:
    image: ghcr.io/cero97-ctrl/sistemahidrico-backend:latest
    # image: ghcr.io/cero97-ctrl/sistemahidrico/backend:latest
    container_name: sistemahidrico-backend
    ports:
  ...
  frontend:
    image: ghcr.io/cero97-ctrl/sistemahidrico-frontend:latest
    # image: ghcr.io/cero97-ctrl/sistemahidrico/frontend:latest
    container_name: sistemahidrico-frontend
    ports:
      - "3000:3000"
    environment:
...
You can feed your own AIS ship data into SistemaHidrico using an RTL-SDR dongle and [AIS-catcher](https://github.com/jvde-github/AIS-catcher), an open-source AIS decoder. This gives you real-time coverage of vessels in your local area — no API key needed.
...
AIS-catcher decodes VHF radio signals on 161.975 MHz and 162.025 MHz and POSTs decoded vessel data to SistemaHidrico every 10 seconds. Ships detected by your SDR antenna appear alongside the global AIS stream.
...
SistemaHidrico is built in the open. These people shipped real code:
