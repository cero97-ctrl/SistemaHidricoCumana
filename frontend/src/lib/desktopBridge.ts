...
import type { SistemaHidricoLocalControlBridge } from '@/lib/localControlTransport';

export interface SistemaHidricoDesktopRuntime {
...
function buildDesktopControlBridge(
  runtime: SistemaHidricoDesktopRuntime,
): SistemaHidricoLocalControlBridge | null {
...
export function installDesktopControlBridge(runtime: SistemaHidricoDesktopRuntime): boolean {
  if (typeof window === 'undefined') return false;
  const bridge = buildDesktopControlBridge(runtime);
  if (!bridge) return false;
  window.__SISTEMAHIDRICO_LOCAL_CONTROL__ = bridge;
  window.__SISTEMAHIDRICO_DESKTOP__ = runtime;
  return true;
}
...
export function getDesktopNativeControlAuditReport(limit?: number): DesktopControlAuditReport | null {
  ...
  const runtime = window.__SISTEMAHIDRICO_DESKTOP__;
