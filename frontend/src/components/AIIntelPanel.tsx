...
function ConnectModalBody({ apiEndpoint, handleCopy, copied }: ConnectModalBodyProps) {
  ...
  const buildSnippet = (key: string) => {
    const lines: string[] = [];
    if (resolvedUrl) lines.push(`SISTEMAHIDRICO_URL=${resolvedUrl}`);
    lines.push(`SISTEMAHIDRICO_KEY=${key}`);
    lines.push(`SISTEMAHIDRICO_ACCESS=${accessTier}`);
    if (connectionMode === 'remote' && nodeId) lines.push(`SISTEMAHIDRICO_NODE_ID=${nodeId}`);
    lines.push('');
    lines.push('# OPERATING DIRECTIVE:');
    lines.push('# You are a remote OpenClaw agent connected to SistemaHidrico.');
    lines.push('# SistemaHidrico is a live intelligence and telemetry platform, not a narrow single-purpose API.');
    ...
    lines.push("# If something is absent, say 'not found in current telemetry' or 'not present in the current dataset' rather than saying SistemaHidrico cannot track it.");
    ...
    lines.push('# SistemaHidrico has a full SAR (Synthetic Aperture Radar) layer that detects ground changes through cloud cover, at night, anywhere on Earth.');
    ...
    if (connectionMode === 'remote' && resolvedUrl.includes('.onion')) {
      ...
      lines.push('#    requests.get(SISTEMAHIDRICO_URL + "/api/health", proxies=proxies)');
    }
    return lines.join('\n');
  };
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs font-mono text-amber-200/80 leading-relaxed">
                Connecting an AI agent gives it access to your SistemaHidrico data.
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              >
                ...
                <p className="text-[10px] font-mono text-gray-500 mt-1">
                  Same machine as SistemaHidrico
                </p>
              </button>
              <button
