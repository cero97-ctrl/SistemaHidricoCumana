...
export default function InfonetShell({
        ...
        setHistory(prev => [...prev, {
          ...
          output: (
            <span className="text-cyan-400">
              ...
              {' '}
              <span className="text-red-400">&apos;sistemahidrico&apos; is reserved.</span>
            </span>
          )
        }]);
        return;
  ...
  const handleCommand = (cmd: string) => {
    ...
    if (inputMode === 'persona') {
      if (trimmedCmd === 'sistemahidrico') {
        output = <span className="text-red-500 font-bold animate-pulse">ERR: Persona &apos;sistemahidrico&apos; is reserved and cannot be claimed.</span>;
