...
const SISTEMAHIDRICO_WELCOME_ID = 'sistemahidrico-welcome';
...
function createSistemaHidricoWelcomeMail(): MailItem {
  return {
    id: SISTEMAHIDRICO_WELCOME_ID,
    msgId: SISTEMAHIDRICO_WELCOME_ID,
    folder: 'inbox',
    kind: 'system',
    direction: 'local',
    senderId: 'sistemahidrico',
    recipientId: 'local',
...
function ensureSeedMail(items: MailItem[]): MailItem[] {
  if (items.some((item) => item.id === SISTEMAHIDRICO_WELCOME_ID)) {
    return sortMessages(items);
  }
  return sortMessages([createSistemaHidricoWelcomeMail(), ...items]);
}
...
function displayNameForPeer(peerId: string, contacts: Record<string, Contact>): string {
  ...
  if (peerId === 'sistemahidrico') return 'sistemahidrico';
...
export default function MessagesView({ onBack, onOpenDeadDrop }: MessagesViewProps) {
  ...
  const handleReply = useCallback((mail: MailItem) => {
    if (!mail.senderId || mail.senderId === 'sistemahidrico' || mail.senderId === 'system') {
      return;
    }
    setDraft({
              ...
              {selectedMessage ? (
                <div className="p-6">
                  ...
                  <div className="mt-6 flex flex-wrap gap-3">
                    {selectedMessage.kind === 'mail' && selectedMessage.senderId !== 'sistemahidrico' && (
                      <button
