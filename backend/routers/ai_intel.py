...
async def ai_status(request: Request):
    ...
    return {
        "ok": True,
        "service": "SistemaHidrico AI Intel",
        "version": "1.0.0",
...
async def ai_satellite_images(
    ...
    def _do_stac_search() -> list[dict]:
        ...
        resp = req.post(
            ...
            json=search_payload,
            timeout=10,
            headers={"User-Agent": "SistemaHidrico-OSINT/1.0 (ai-intel)"},
        )
...
async def inject_data(request: Request, body: InjectRequest):
    """Inject custom data into ANY native SistemaHidrico layer.
...
async def generate_report(request: Request):
    ...
    report = {
        ...
        "title": f"SistemaHidrico Intelligence Report — {now.strftime('%Y-%m-%d %H:%M')} UTC",
        "summary": {
...
async def agent_tool_manifest(request: Request):
    ...
    return {
        ...
        "tips": [
            ...
            "SistemaHidrico does expose UAP sightings, wastewater, and tracked_flights/VIP aircraft when those layers are populated. Verify with get_summary or get_layer_slice before claiming a layer is absent.",
            "SistemaHidrico also exposes fishing_activity, which is the fishing-vessel activity layer backed by Global Fishing Watch data when GFW_API_TOKEN is configured. Do not confuse it with the AIS ships layer.",
