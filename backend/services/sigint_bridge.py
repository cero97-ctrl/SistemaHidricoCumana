...
class APRSBridge:
    ...
    LOGIN = "user N0CALL pass -1 vers SistemaHidrico 1.0 filter r/0/0/25000\r\n"
    CONFIDENCE = 0.7
...
class MeshtasticBridge:
    ...
    def _build_client_id() -> str:
        ...
        SistemaHidrico instances kick each other off the broker.

        This is deliberately not tied to the user's public mesh address or
        SistemaHidrico node identity; it is only an MQTT session handle.
        """
...
def send_aprs_message(callsign: str, passcode: str, target: str, message: str) -> dict:
    ...
    port = 14580
    login = f"user {callsign} pass {passcode} vers SistemaHidrico 1.0\r\n"
