# SistemaHidrico Helm Chart

A Helm chart for deploying SistemaHidrico services (backend and frontend).
...
```bash
helm install sistemahidrico ./chart --create-namespace
```

Or use the repository:

```bash
helm install sistemahidrico bjw-s-labs/app-template \
  --namespace sistemahidrico \
  -f values.yaml
```
...
| `controllers.backend.containers.main.image.repository` | Container image | `registry.gitlab.com/cero97-ctrl/sistemahidrico/backend` (or `ghcr.io/cero97-ctrl/sistemahidrico-backend`) |
...
| `controllers.frontend.containers.main.image.repository` | Container image | `registry.gitlab.com/cero97-ctrl/sistemahidrico/frontend` (or `ghcr.io/cero97-ctrl/sistemahidrico-frontend`) |
...
```bash
helm uninstall sistemahidrico -n sistemahidrico
```
